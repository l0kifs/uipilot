...
_ELEMENT_ID_RE = re.compile(r"\b([a-z]{2,5})_((?:btn|input|toast|dialog|select|link|"
...
def _merge_structured(blocks: list[dict], result: ImportResult) -> None:
    for block in blocks:
        ...
        if app_hdr and isinstance(app_hdr, dict) and app_hdr.get("id"):
            ...
            entry = result.apps.setdefault(block_app,
                                           {"app": app_hdr, "elements": {}, "actions": {}})
            entry["app"] = app_hdr
...
def _harvest_heuristic(text: str, result: ImportResult) -> None:
    ...
    for m in _ACTION_ID_RE.finditer(text):
        ...
        entry = result.apps.setdefault(prefix, {"app": None, "elements": {}, "actions": {}})
        entry["actions"].setdefault(aid, {
            "app": prefix, "purpose": "TODO seeded from MD", "risk": "low",
            "elements": [], "prev": [], "next": [], "steps": [],
        })
...
_BANNER = ("# SEED — generated by `uipilot import-md`. Review before use:\n"
...
def write_seed(result: ImportResult, out_dir: str | Path) -> list[Path]:
    ...
    if result.flows:
        ...
        flows_path.write_text(_BANNER + yaml.safe_dump({"flows": result.flows}, sort_keys=False),
                              encoding="utf-8")
    ...
    config = {
        ...
        "tokens": {"prefix": {"from": "env", "name": "TEST_ENTITY_PREFIX", "default": "uipilot"},
                   "seq": {"from": "counter"}},
        "risk": {"levels": ["low", "admin-control", "credential", "destructive", "money-moving"],
