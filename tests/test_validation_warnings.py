...
def test_unknown_risk_error(tmp_path):
    pack = _broken(tmp_path, lambda root: _edit_console(
...
def test_bad_api_role_error(tmp_path):
    pack = _broken(tmp_path, lambda root: _edit_flows(
...
def test_no_steps_warning(tmp_path):
    def mutate(root):
        _edit_console(root, lambda d: d["actions"]["act_cs_view_dashboard"].__setitem__("steps", []))
    assert "W_NO_STEPS" in _codes(_broken(tmp_path, mutate))


def test_no_capture_warning(tmp_path):
    def mutate(root):
        _edit_console(root, lambda d: d["actions"]["act_cs_create_project"]["steps"][2]
                      .__setitem__("value", "{{captured.nobody_makes_this}}"))
    ...
    def mutate(root):
        def fn(d):
            ...
            d["actions"]["act_cs_credentials_twin"] = {
                "purpose": "twin", "route": "/projects",
                "elements": src["elements"], "prev": [], "next": [],
                "steps": src["steps"],
            }
    ...
    def mutate(root):
        ...
        def fn(d):
            d["actions"]["act_cs_island_a"] = {
                "purpose": "island", "route": "/x", "elements": [], "steps": [],
                "prev": ["act_cs_island_b"], "next": ["act_cs_island_b"]}
            d["actions"]["act_cs_island_b"] = {
                "purpose": "island", "route": "/x", "elements": [], "steps": [],
                "prev": ["act_cs_island_a"], "next": ["act_cs_island_a"]}
        _edit_console(root, fn)
    ...
    def mutate(root):
        ...
        def fn(d):
            d["actions"]["act_cs_orphan_project"] = {
                ...
                "route": "/projects", "elements": [], "steps": [],
                "prev": [], "next": [], "provides": ["project"]}
        _edit_console(root, fn)
...
def test_app_scope_filters_out_of_scope_findings(tmp_path):
    def mutate(root):
        _edit_console(root, lambda d: d["actions"]["act_cs_create_project"]["elements"]
                      .append("cs_ghost"))
